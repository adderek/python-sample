...
# Normally you shouldn't disable this
# But we have example method with single function only
...
# Classes should not contain variables
#   for this you should use instances
# Modules are to be used with single instance...
#   but you could also create local "copies"
# Classes allow one class to inherit from another

# Examples:
#  math is a good item for a module
#    you have all the things not to be "call specific"
#  logging is a thing to be class-like
#    you should never re-use same instance in different places
...
class Example:
    ...
    def __init__(self, name):
        ...
        # (other instances will have its own)

        # local_variable = 'this variable will disappear after
        # the method (constructor in this case)'  # never used variable

        # We can use variables in a string in 3+ ways
        # Whenever using loggers you should always pass the variables
        # as extra values to let logger parse
        #  because those are not always shows
        #    (depends on the logger configuration)
        #  and tools can access it in other way than simple strings
        logger.debug("New instance of Example class with name='{}'", name)
        # logger.debug("New instance of Example class with name='%s'"
        #              % (name))
        # logger.debug("New instance of Example class with name='{}'"
        #              .format(name))
        # logger.debug(f"New instance of Example class with name='{name}'")
    ...
    def __str__(self):
        ...
        return (
            f"<class Example>"
            f"({self.class_variable}, {self.instance_variable})"
            )
