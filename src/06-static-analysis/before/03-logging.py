...
# Don't reinvent the wheel... or do it as a separate project and share over the internet
...
def main_using_python_logger():
    logging.basicConfig(
            ...
            format='%(asctime)s|%(levelname)s|%(message)s',
                # Rule of a thumb: always use international standard formats. ISO-8601 is a document worth reading.
                #  asctime is not following that rule
            datefmt='%Y-%m-%dT%H:%M:%S')
                # Custom format... does not support fractional part
    # But we can create our own "keywords" like %(isotime)s - but this shows that logging is a poor implmementation :(
...
def main_using_loguru():
    ...
    logger.debug("World")  # This message will be printed to stderr using default sink and again using second sink
